function* getDSTTransitions_without_Intl(tz_name, date_from, date_end) {
	date_from ??= new Date()
	...
	let m = date_from.getUTCMonth() - 1, d_1 = Date.UTC(y, m), p_1 = new Date(d_1).toLocaleTimeString('en-US', opts)
	for (let d_2; (d_2 = Date.UTC(y, ++m)) < end;) {
		let p_2 = new Date(d_2).toLocaleTimeString('en-US', opts)
		if (p_1 !== p_2) {
			...
			while ((step = (t_3 - t_1) >> 1)) {
				const t_2 = t_1 + step
				const p_3 = new Date(t_2 * 1000).toLocaleTimeString('en-US', opts)
